...
function MainView({ active, username }) {
  ...
  const [programmingLanguage, setLanguage] = useState("C#");
  const [phase, setPhase] = useState("Code Generation");
  ...
  const suggestionMap = {
    ...
    "/": ["java", "python", "c#","Kotlin","Flutter","android",],
  };
  ...
  const getOptions = () => {
    ...
    return (
      <>
        ...
        <option value="Private Repo">Private Repo
        </option>
      </>
    );
    //} 
  ...
  const handleEditClick = ({ sender, text }) => {
    console.log('hhhhhhhhhhhhhhhhhhhh',editprompt)
  ...
  const MessageBubbleInner = ({ id, text, sender, time, feedback, chatid }) => {
        ...
        <div className="">
          {hoveredMessageId === id && (
            <div
              ...
              style={{ paddingLeft: "min(125px, 55%)", marginTop: "-20px" }}
            >
              {sender === "user" && (
                <FiEdit
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      const response = await fetch(`${config.apiBaseUrl}file/upload`, {
        method: "POST",
        body: formData,
      });
      ...
      console.error('qqqqqqqqqqqqqqqqqqq',error);
    }
  }
  console.log('=============apiResponse', apiResponse);
    ...
    let language = firstInput[0].replace("#", "");
    language = language.replace("-", " ");
    let phase = firstInput[1].replace("@", "");

    phase = phase.replace("-", " ");

    let text = secondInput;
    if (phase == "convert") {
      text = secondInput.replace("/", "");
    }
    console.log("aaaaaaaaaaaaaaaaaaaaaaaaaaaaa",referenceCode)
  ...
  <path d="M4.5 3a2.5 2.5 0 0 1 5 0v9a1.5 1.5 0 0 1-3 0V5a.5.5 0 0 1 1 0v7a.5.5 0 0 0 1 0V3a1.5 1.5 0 1 0-3 0v9a2.5 2.5 0 0 0 5 0V5a.5.5 0 0 1 1 0v7a3.5 3.5 0 1 1-7 0z"/>
                ...
                />
                {selectVisible ? (
                  <select classname="form-select" onChange={handleSelectChange} aria-label="Default select example" style={{ marginLeft: "5px", transform: "translateY(-120%)", width: "20%", float: "right" }}>
                ...
                <div>
                  <div style={{ float: "left" }}>
                    {error ? (
                      ...
                      >
                        Please enter # for Programming language, @ for Phase and
                        prompt or / for convert serial wise
                      </p>
                    ) : (
                      ""
                    )}
